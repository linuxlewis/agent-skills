...
- The public preview requires no repository setting or enablement on supported GitHub.com
  repositories. Exit **9** means the current host or repository does not expose Stacked PRs; report
  that limitation rather than looking for a setting to turn on. Under a TTY the command may offer
  to create ordinary unstacked PRs instead, but that does not satisfy a request for a native Stack.
